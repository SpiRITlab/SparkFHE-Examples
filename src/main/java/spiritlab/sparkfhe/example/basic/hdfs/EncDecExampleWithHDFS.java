...
public class EncDecExampleWithHDFS {
    ...
    public static void main(String args[]) {
        ...
        String CTXT_0_FILE = Config.get_local_HDFS_path("/ptxt_long_0_"+ SparkFHE.getInstance().generate_crypto_params_suffix()+ ".json");
        String CTXT_1_FILE = Config.get_local_HDFS_path("/ptxt_long_1_"+SparkFHE.getInstance().generate_crypto_params_suffix()+ ".json");
        ...
        for (int l=0;l<2;l++) {
            ...
            SparkFHE.getInstance().store_ciphertext_to_file(
                    ...
                    Config.get_local_HDFS_path("ptxt_long_"+String.valueOf(l)+"_"+SparkFHE.getInstance().generate_crypto_params_suffix()+ ".json"));
        }
        ...
        SparkFHE.getInstance().store_ciphertexts_to_file(DataSourceHandler.Ciphertext_Label, vec_ctxt_1, Config.get_local_HDFS_path("vec_a_"+String.valueOf(Config.NUM_OF_VECTOR_ELEMENTS)+"_"+SparkFHE.getInstance().generate_crypto_params_suffix()+ ".json"));

        // encrypt them and store to pre-defined location
        vec_ctxt_2=SparkFHE.getInstance().encrypt(vec_ptxt_2);
        SparkFHE.getInstance().store_ciphertexts_to_file(DataSourceHandler.Ciphertext_Label, vec_ctxt_2, Config.get_local_HDFS_path("vec_b_"+String.valueOf(Config.NUM_OF_VECTOR_ELEMENTS)+"_"+SparkFHE.getInstance().generate_crypto_params_suffix()+ ".json"));
