...
//            SparkFHEPlugin.setup();
    ...
    public static void test_FHE_total_sum_via_lambda(SparkSession spark, int slices, String library, String scheme, Broadcast<String> pk_b,
        ...
        SerializedCiphertext res = ctxt_vec_rdd.reduce((x, y) -> {
            // we need to load the shared library and init a copy of SparkFHE on the executor
//            SparkFHEPlugin.setup();
...
//        org.apache.spark.mllib_fhe.linalg.CtxtVector inputVector = vec;
        ...
        endTime = System.currentTimeMillis();
        System.out.println("TIMEINFO:FHE_total_sum_via_spark_integration:" + (endTime - startTime) + ":ms");
            ...
            transformedData.foreach(v -> {
                AbstractFunction2<Object, String, BoxedUnit> f = new AbstractFunction2<Object, String, BoxedUnit>() {
                    public BoxedUnit apply(Object t1, String t2) {
                        if((int)t1==0)
                            System.out.println("transformedData Total-Sum:" + SparkFHE.getInstance().decrypt(new Ciphertext((String)t2), true));
...
//            SparkFHE.init(library, scheme, pk_b.getValue(), sk_b.getValue(), rlk_b.getValue(), glk_b.getValue());
        ...
        endTime = System.currentTimeMillis();
        System.out.println("TIMEINFO:FHE_total_sum_via_sql:" + (endTime - startTime) + ":ms");
...
//        System.out.println("TIMEINFO:FHE_total_Spark_total_sum_job_via_lambda:" + (main_endTime - main_startTime) + ":ms");
